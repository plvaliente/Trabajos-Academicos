...
void planificadorCon3()
{
  PlanificadorRR<int> planificador;
  planificador.agregarProceso(1);
  planificador.agregarProceso(2);
  planificador.agregarProceso(3);
  ASSERT_EQ(planificador.hayProcesos(), true);
  ASSERT_EQ(planificador.procesoEjecutado(),1)
  ASSERT_EQ(planificador.hayProcesosActivos(), true);
  ASSERT_EQ(to_s(planificador), "[1*, 2, 3]");
  planificador.ejecutarSiguienteProceso();
  ASSERT_EQ(to_s(planificador), "[2*, 3, 1]");
  planificador.pausarProceso(2);
  ASSERT_EQ(to_s(planificador), "[3*, 1, 2 (i)]");
  ASSERT_EQ(planificador.hayProcesosActivos(),true);
  planificador.reanudarProceso(2);
  ASSERT_EQ(to_s(planificador), "[3*, 1, 2]");
}

void planificadorPorCopia(){
  PlanificadorRR<int> planificador1 ;
  planificador1.agregarProceso(1);
  planificador1.agregarProceso(2);
  planificador1.pausarProceso(2);
  ASSERT_EQ(to_s(planificador1), "[1*, 2 (i)]");
  PlanificadorRR<int> planificador2 = PlanificadorRR<int>(planificador1);
  ASSERT_EQ(to_s(planificador2), "[1*, 2 (i)]");
  planificador2.reanudarProceso(2);
  ASSERT_EQ(to_s(planificador2), "[1*, 2]");
  ASSERT_EQ(to_s(planificador1), "[1*, 2 (i)]");

}

void planificadorIgual(){
  PlanificadorRR<int> planificador1;
  planificador1.agregarProceso(1);
  planificador1.agregarProceso(2);
  planificador1.pausarProceso(2);
  ASSERT_EQ(to_s(planificador1), "[1*, 2 (i)]");
  PlanificadorRR<int> planificador2 = PlanificadorRR<int>(planificador1);
  assert(planificador1==planificador2);
}

void planificadorDePlanificadores(){
  PlanificadorRR< PlanificadorRR<int> > planificador;
  PlanificadorRR<int> planificador1;
  PlanificadorRR<int> planificador2;
  planificador1.agregarProceso(1);
  planificador1.agregarProceso(2);
  planificador2.agregarProceso(3);
  planificador2.agregarProceso(4);
  planificador.agregarProceso(planificador1);
  planificador.agregarProceso(planificador2);
  ASSERT_EQ(to_s(planificador), "[[1*, 2]*, [3*, 4]]");
}
...
void planificadorDec()
{
  ...
  planificador.ejecutarSiguienteProceso();
  PlanificadorRR<int> copia = PlanificadorRR<int>(uno);
  PlanificadorRR<int> copia1 = PlanificadorRR<int>(planificador);
  copia1.detener();
  ASSERT_EQ(copia1.detenido(), true);
  copia1.reanudar();
  ASSERT_EQ(copia1.detenido(), false);
  ASSERT_EQ( to_s(copia1), to_s(planificador));
...
void test_copia() {
    ...
    PlanificadorRR<int> p2 = PlanificadorRR<int>(p1); //prueba constructor por copia y ==
...
int main()
{
  ...
  RUN_TEST( planificadorCon3 );
  RUN_TEST( planificadorPorCopia );
  RUN_TEST( planificadorIgual );
  RUN_TEST( planificadorDePlanificadores );
