...
//VERSION 9: incluye un iterador para listas constantes.  Falta para listas modificables.
//
...
public:
    ...
    class iterador {
        const Nodo* _actual;
        const lista* _lista;
        iterador(const lista* l)  {
            _lista = l;
            _actual = l->prim;
        }
        //para poder construir el iterador
        friend class lista;
    public:
        /** Constructor por defecto; permite declarar un iterador sin inicializar
         * Ojo: su uso esta indefinido
         */
        iterador() {
            _lista = nullptr; _actual = nullptr;
        }
        
        /** Avanza el iterador.  Requiere hay_actual() */
        void avanzar() {
            assert(hay_actual());
            _actual = _actual->siguiente;
            if(_actual == _lista->prim) _actual = nullptr;
        }
        
        /** True si hay otra posicion.  Ojo: el const es del iterador */
        bool hay_actual() const {
            return _actual != nullptr;
        }
        
        /** Retorna el elemento actual.  El const de la derecha es del iterador! */
        const Elem& actual() const {
            assert(hay_actual());
            return _actual->elem;
        }
    };
    
    /** Funcion que retorna un nuevo iterador */
    iterador dame_iterador() const {
...
template<class Elem>
bool esta(const lista<Elem>& l, const Elem& x) {
    //en C++11 conviene usar auto.  El tipo verdadero es:
    //typename lista<Elem>::iterador (no explico el porqué del typename).
    auto it = l.dame_iterador();
    while(it.hay_actual() && it.actual() != x) {
        it.avanzar();
    }
    return it.hay_actual();
}
