...
//VERSION 7: hace una sobrecarga de las funciones que depende de si el tipo de this es const o no.  Muestra que, efectivamente,
//la clase lista define *dos* tipos por cada tipo Elem: lista<Elem> y const lista<Elem>.  Los lvalues bindean tanto con
//lista<Elem>& (en cuyo caso puedo invocar funciones que modifiquen la lista) como con const lista<Elem>& (en cuyo caso me
//comprometo a no modificar la lista).  En cambio, los rvalues solo bindean con const lista<Elem>&, ya que *no* puedo modificar
//la lista (es lo mismo que pasaba con el constness de Elem).
//A pesar de todo esto, esta version hace algunas copias innecesarias debido a restricciones de C++03.
//
...
class lista {
    ...
    struct Nodo {
        ...
        Nodo(const Elem& e, Nodo* p, Nodo* a) : elem(e) { //invoca al constructor por copia
