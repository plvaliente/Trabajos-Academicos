...
public:
	...
	void push_front(const T& val) {
		...
		/*
		  Si nadie cambió la cabeza, entonces en newNode->next pongo la vieja cabeza.
		  Si otro thread la cambió, head ya no es igual a newNode->next, por lo la función devuelve false,
		  entra en el ciclo porque está negado y guarda head (el nodo que insertó el otro thread) en newNode->next,
		  y vuelve a probar en la siguiente iteración.
		 */
		while(!std::atomic_compare_exchange_weak_explicit(&_head, &new_node->_next, new_node,
	...
	Iterador CrearIt() {
		return Iterador(this, _head.load());
	}
};
